...
class _DriverTripsScreenState extends State<DriverTripsScreen>
  ...
  final List<Map<String, dynamic>> _mockTransport = [
    {
      'id': 201, 'request_number': 'TR-20260603-99V5',
      'pickup_location': 'Yeka Subcity Store A', 'destination': 'Gotera Warehouse 3',
      'passenger_item_details': '50 packages cleaning detergents',
      'requested_date': '2026-06-03', 'requested_time': '09:00',
      'status': 'DriverAssigned', 'mahberat_name': 'Yeka Logistic Corp',
      'special_instructions': 'Verify seals before loading.',
    },
    {
      'id': 202, 'request_number': 'TR-20260603-FF22',
      'pickup_location': 'Gerji Condominium Gate 2', 'destination': 'Cleaning Plant 1',
      'passenger_item_details': '3 municipal sweepers staff',
      'requested_date': '2026-06-03', 'requested_time': '11:15',
      'status': 'PickedUp', 'mahberat_name': 'Gerji Cleaners Union',
  ...
  Future<void> _loadTrips() async {
    ...
    try {
      ...
      setState(() { _transportTrips = _mockTransport; _isLoading = false; });
    }
  }
    ...
    switch (_filterStatus) {
      case 'active':
        return _transportTrips.where((t) {
          final s = t['status'] ?? '';
          return ['DriverAssigned','DriverAccepted','PickedUp','MahberatApprovedPickup']
              .contains(s);
        }).toList();
      case 'done':
        return _transportTrips.where((t) {
          final s = t['status'] ?? '';
          return ['ReceiptSubmitted','ReceiptVerified','StaffApproved','Paid',
              'DispatcherRejected','DriverRejected','StaffRejected'].contains(s);
        }).toList();
      default:
  ...
  Future<void> _submitReceipt() async {
    ...
    if (_selWereda == null || _selEntity == null) {
      _snack('Please select Wereda and ${_receiptType == 'Mahberat' ? 'Mahberat' : 'Company'}');
      return;
    }
    ...
    final sub = SubmissionModel(
      ...
      weredaId: _selWereda!, mahberatId: _selEntity!,
    ...
    if (online) {
      ...
      if (!ok) { await LocalDbService().saveOfflineSubmission(sub); offline = true; }
    } else {
      await LocalDbService().saveOfflineSubmission(sub); offline = true;
    }

    if (mounted) {
      setState(() { _submitting = false; _images = []; _kgCtrl.clear(); _notesCtrl.clear(); });
      _snack(ok ? 'Receipt submitted!' : offline ? 'Saved offline â€” will sync later.' : 'Submission failed.',
          color: ok ? Colors.green : offline ? Colors.orange : Colors.red);
    }
  }
  ...
  Future<void> _accept(Map<String, dynamic> trip) async {
    final user = Provider.of<AuthService>(context, listen: false).currentUser!;
    setState(() => _isLoading = true);
    final ok = await _apiService.acceptTransportRequest(trip['id'],
        driverId: user.id, driverName: user.name);
    if (ok) { _snack('Job Accepted', color: Colors.green); }
    else    { trip['status'] = 'DriverAccepted'; _snack('Simulated: Accepted (offline)'); }
  ...
  Future<void> _reject(Map<String, dynamic> trip) async {
    final user = Provider.of<AuthService>(context, listen: false).currentUser!;
    setState(() => _isLoading = true);
    final ok = await _apiService.rejectTransportRequest(trip['id'],
        driverId: user.id, driverName: user.name);
    if (ok) { _snack('Job Rejected', color: Colors.orange); }
    else    { _transportTrips.removeWhere((t) => t['id'] == trip['id']); _snack('Simulated: Rejected (offline)'); }
    setState(() => _isLoading = false);
    _loadTrips();
  }
  ...
  Future<void> _pickup(Map<String, dynamic> trip) async {
    final user = Provider.of<AuthService>(context, listen: false).currentUser!;
    setState(() => _isLoading = true);
    final ok = await _apiService.markTransportPickedUp(trip['id'],
        driverId: user.id, driverName: user.name, notes: 'Payload loaded');
    if (ok) { _snack('Pickup Confirmed', color: Colors.blue); }
    else    { trip['status'] = 'PickedUp'; _snack('Simulated: Picked up (offline)'); }
    setState(() => _isLoading = false);
    _loadTrips();
  }
  ...
  Future<void> _submitReceiptPhoto(Map<String, dynamic> trip) async {
    ...
    await showModalBottomSheet(
      ...
      builder: (_) => _TransportReceiptSheet(
        trip: trip, user: user, apiService: _apiService,
        weredas: _weredas, mahberats: _mahberats,
        onDone: (ok) {
          _snack(ok ? 'Receipt Submitted!' : 'Saved offline  will sync later.',
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(icon: Icon(Icons.local_shipping_outlined), text: 'Transport Jobs'),
            Tab(icon: Icon(Icons.receipt_long_outlined),   text: 'Submit Receipt'),
          ],
        ),
      ),
      body: TabBarView(
              ...
              : RefreshIndicator(
                  ...
                  child: _filteredTrips.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (_, i) => _buildTripCard(_filteredTrips[i], isDark)
                              .animate()
  ...
  Widget _buildTripCard(Map<String, dynamic> trip, bool isDark) {
    ...
    final ts = isDark ? Colors.grey[400]! : (AppColors.textSecondary ?? Colors.grey[600]!);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _row(Icons.location_on,   'Pickup',       trip['pickup_location'] ?? '', tp, ts),
            const SizedBox(height: 6),
            _row(Icons.flag_rounded,  'Destination',  trip['destination'] ?? '', tp, ts),
            const SizedBox(height: 6),
            _row(Icons.backpack_rounded, 'Cargo',    trip['passenger_item_details'] ?? '', tp, ts),
            const SizedBox(height: 6),
            _row(Icons.calendar_today, 'Schedule',
                '${trip['requested_date'] ?? ''} @ ${trip['requested_time'] ?? ''}', tp, ts),
            if ((trip['special_instructions'] ?? '').toString().isNotEmpty) ...[
              const SizedBox(height: 6),
              _row(Icons.info_outline, 'Notes', trip['special_instructions'] ?? '', tp, ts),
            ],
  ...
  Widget _buildStepTracker(String status) {
    final steps = [
      ('Assigned', 'Dispatcher'), ('Accepted', 'You'),
      ('Picked Up', 'You'), ('Receipt', 'You'), ('Paid', 'Finance'),
    ];
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            Text('Rejected', style: TextStyle(color: Colors.red[700], fontWeight: FontWeight.bold, fontSize: 12)),
          ],
        ),
      );
    }
    ...
    switch (status) {
      ...
      case 'DriverAccepted':
        return SizedBox(width: double.infinity, child: ElevatedButton.icon(
          ...
          label: const Text('Confirm Payload Loaded', style: TextStyle(color: Colors.white)),
      ...
      case 'PickedUp':
        ...
        return SizedBox(width: double.infinity, child: ElevatedButton.icon(
          ...
          label: const Text('Submit Receipt (KG + Photos)', style: TextStyle(color: Colors.white)),
      ...
      default:
        ...
        final statusMsgs = {
          'ReceiptSubmitted': ('Receipt submitted — awaiting Mahberat review.', Colors.orange),
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            Icon(
              status == 'Paid' ? Icons.check_circle : Icons.hourglass_bottom_rounded,
              color: info?.$2 ?? Colors.teal, size: 16),
            const SizedBox(width: 6),
            Expanded(child: Text(info?.$1 ?? 'Awaiting next action.',
                style: TextStyle(color: info?.$2 ?? Colors.teal,
              ...
              child: Column(
                children: [
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    value: _selEntity,
                    items: (_receiptType == 'Mahberat' ? _mahberats : _companies)
                  ...
                  if (user?.isDriver == true)
                    DropdownButtonFormField<int>(
                      decoration: _dec('Vehicle (Optional)', Icons.local_shipping),
                      value: _selVehicle,
                  ...
                  child: Container(
                    ...
                    child: _images.isNotEmpty
                              ...
                              builder: (_, snap) => snap.hasData
                                  ? Padding(
                                      ...
                                      child: ClipRRect(
                                        ...
                                        child: Image.memory(snap.data!, width: 100, fit: BoxFit.cover),
                                      ))
                        ...
                        : const Column(
                            ...
                            children: [
                              Icon(Icons.add_a_photo_outlined, size: 36, color: AppColors.textHint),
                              SizedBox(height: 6),
  ...
  Widget _typeBtn(String label, String value) {
    ...
    return GestureDetector(
      onTap: () => setState(() { _receiptType = value; _selEntity = null; }),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 200),
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: sel ? [const BoxShadow(color: Colors.black12, blurRadius: 4, offset: Offset(0, 2))] : null,
        ),
  ...
  Widget _row(IconData icon, String label, String val, Color tp, Color ts) => Row(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      ...
      Expanded(child: RichText(text: TextSpan(
        text: '$label: ',
        style: TextStyle(fontWeight: FontWeight.w600, fontSize: 12, color: ts, fontFamily: 'Inter'),
        children: [TextSpan(text: val, style: TextStyle(fontWeight: FontWeight.normal, color: tp))],
      ))),
    ],
  );
...
class _TransportReceiptSheetState extends State<_TransportReceiptSheet> {
  ...
  void initState() {
    ...
    if (wid != null && widget.weredas.any((w) => w['id'] == wid)) {
      ...
      final wName = (t['wereda_name'] ?? t['wereda'] ?? '').toString().toLowerCase().trim();
      if (wName.isNotEmpty) {
    ...
    if (mid != null && widget.mahberats.any((m) => m['id'] == mid)) {
      ...
      final mName = (t['mahberat_name'] ?? t['mahberat'] ?? '').toString().toLowerCase().trim();
      if (mName.isNotEmpty) {
    ...
    final kgMatch = RegExp(r'(\d+(?:\.\d+)?)\s*(?:kg|kilo)', caseSensitive: false).firstMatch(details);
    if (kgMatch != null) {
  ...
  @override
  void dispose() { _kgCtrl.dispose(); _notesCtrl.dispose(); super.dispose(); }
  ...
  Future<void> _submit() async {
    ...
    final ok = await widget.apiService.submitTransportReceipt(
      ...
      notes: urls.length > 1
          ? '${_notesCtrl.text.trim()} | extra_photos:${urls.skip(1).join(',')}'.trim()
          : _notesCtrl.text,
    ...
    if (mounted) { Navigator.pop(context); widget.onDone(ok); }
  }
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Expanded(child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Submit Trip Receipt', style: TextStyle(
                        color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
                    Text(trip['request_number'] ?? '',
                        style: TextStyle(
              ...
              Wrap(spacing: 8, runSpacing: 6, children: [
                ...
                _infoChip(Icons.local_shipping_outlined, user.vehicleName ?? 'No Vehicle'),
                _infoChip(Icons.location_on_outlined, trip['pickup_location'] ?? ''),
            ...
            child: Form(key: _formKey, child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      ...
                      const SizedBox(width: 6),
                      const Text('TRANSPORT REQUEST INFO',
                          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                              color: AppColors.primary, letterSpacing: 0.8)),
                    ]),
                    const SizedBox(height: 10),
                    _tripInfoRow(Icons.location_on_outlined,    'Pickup',      trip['pickup_location'] ?? '—'),
                    const SizedBox(height: 6),
                    _tripInfoRow(Icons.flag_outlined,           'Destination', trip['destination'] ?? '—'),
                    const SizedBox(height: 6),
                    _tripInfoRow(Icons.backpack_outlined,       'Cargo',       trip['passenger_item_details'] ?? '—'),
                    if ((trip['mahberat_name'] ?? '').toString().isNotEmpty) ...[
                      const SizedBox(height: 6),
                      _tripInfoRow(Icons.business_outlined,    'Mahberat',    trip['mahberat_name'] ?? '—'),
                    ],
                    const SizedBox(height: 6),
                    _tripInfoRow(Icons.calendar_today_outlined, 'Schedule',
                        '${trip['requested_date'] ?? ''} @ ${trip['requested_time'] ?? ''}'),
                    if ((trip['special_instructions'] ?? '').toString().isNotEmpty) ...[
                      const SizedBox(height: 6),
                      _tripInfoRow(Icons.info_outline, 'Instructions', trip['special_instructions'] ?? ''),
                    ],
                  ]),
                ),
                ...
                TextFormField(
                  ...
                  validator: (v) => (v == null || v.isEmpty) ? 'Enter actual KG' : null,
                ),
                ...
                if (_selWereda != null || _selMahberat != null)
                  Container(
                    ...
                    child: Row(children: [
                      Icon(Icons.auto_fix_high, size: 14, color: Colors.green[700]),
                      const SizedBox(width: 6),
                      Text('Auto-filled from transport request',
                ...
                DropdownButtonFormField<int>(
                  decoration: _dec('Wereda', Icons.location_on),
                  value: _selWereda,
                  items: widget.weredas.map((w) =>
                      DropdownMenuItem<int>(value: w['id'], child: Text(w['name']))).toList(),
                  onChanged: (v) => setState(() => _selWereda = v),
                  validator: (v) => v == null ? 'Required' : null,
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<int>(
                  ...
                  value: _selMahberat,
                  items: widget.mahberats.map((m) =>
                      DropdownMenuItem<int>(value: m['id'], child: Text(m['name']))).toList(),
                ...
                if (_photos.isNotEmpty) ...[
                  ...
                  Padding(
                    ...
                    child: Text('${_photos.length} photo(s) selected',
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Trip & Receipt Details', style: TextStyle(
              ...
              Container(
                ...
                child: Text(
                  status.replaceAllMapped(RegExp(r'([A-Z])'), (m) => ' ${m[0]}').trim(),
                  style: TextStyle(color: statusColor == Colors.green ? Colors.white
                      : Colors.white, fontSize: 11, fontWeight: FontWeight.w700),
                ),
              ),
            ]),
          ),
          ...
          Expanded(child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                child: Column(children: [
                  _row('Pickup Location',    trip['pickup_location'],         icon: Icons.location_on),
                  _row('Destination',        trip['destination'],              icon: Icons.flag_rounded),
                  _row('Cargo / Passengers', trip['passenger_item_details'],   icon: Icons.backpack_rounded),
                  _row('Scheduled',
                      '${trip['requested_date'] ?? ''} @ ${trip['requested_time'] ?? ''}',
                      icon: Icons.calendar_today),
                  if ((trip['driver_name'] ?? '').toString().isNotEmpty)
                    _row('Driver',           trip['driver_name'],              icon: Icons.person_outline),
                  if ((trip['vehicle_plate'] ?? '').toString().isNotEmpty)
                    _row('Vehicle',          trip['vehicle_plate'],            icon: Icons.local_shipping_outlined),
                ]),
              ),
              const SizedBox(height: 20),
              ...
              Container(
                ...
                child: Column(children: [
                  _row('Actual Weight (KG)',
                      trip['actual_kilogram'] != null
                          ? '${trip['actual_kilogram']} KG' : null,
                      icon: Icons.scale),
                  _row('Receipt Notes',  trip['receipt_notes'],    icon: Icons.notes),
                  _row('Transport Cost',
                      trip['transport_cost'] != null
                          ? 'ETB ${trip['transport_cost']}' : null,
                      icon: Icons.payments_outlined),
                  _row('Transaction #',  trip['transaction_number'], icon: Icons.tag),
                ]),
              ),

              // ── Receipt photo if available ─────────────────────
              if ((trip['receipt_photo_url'] ?? '').toString().isNotEmpty) ...[
                const SizedBox(height: 20),
              ...
              Container(
                ...
                child: Row(children: [
                  Icon(status == 'Paid' ? Icons.check_circle : Icons.schedule,
                      color: statusColor, size: 24),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(status.replaceAllMapped(
                        RegExp(r'([A-Z])'), (m) => ' ${m[0]}').trim(),
                        style: TextStyle(color: statusColor,
                            fontWeight: FontWeight.w800, fontSize: 15)),
                    Text({
                      'ReceiptSubmitted': 'Mahberat is reviewing your receipt.',
                      'ReceiptVerified':  'Staff is approving the payment.',
                      'StaffApproved':    'Finance is processing payment.',
                      'Paid':             'Payment has been completed.',
                    }[status] ?? '',
                    style: TextStyle(color: statusColor.withValues(alpha: 0.8),
                        fontSize: 12)),
                  ])),
                ]),
              ),
